...
namespace math
{
	...
	template<typename T>
	[[nodiscard]] const T RoundOff(const T& value, int digit)
	{
		// 0桁目は存在しないためそのまま返す
	...
	template<>
	inline [[nodiscard]] const VECTOR RoundOff(const VECTOR& value, int digit)
	{
		return VGet(RoundOff(value.x, digit), RoundOff(value.y, digit), RoundOff(value.z, digit));
	}

	/// @brief クォータニオンから回転行列(回転順は問わない)へ変換
	[[nodiscard]] const MATRIX ConvertQuaternionToRotMatrix(const Quaternion& q);

	/// @brief 回転行列(回転順は問わない)からクォータニオンへ変換
	[[nodiscard]] const Quaternion ConvertRotMatrixToQuaternion(const MATRIX& rot_matrix);

	/// @brief XYZ軸から回転行列(回転順は問わない)へ変換
	[[nodiscard]] const MATRIX ConvertAxisToRotMatrix(const Axis& axis);

	/// @brief XYZ軸からオイラー角へ変換
	[[nodiscard]] const VECTOR ConvertAxisToEulerAngles(const Axis& axis);

	/// @brief 回転行列からオイラー角へ変換
	/// @brief FIXME : 不具合ありな可能性あり。要検証
	[[nodiscard]] const VECTOR ConvertXYZRotMatrixToEulerAngles(const MATRIX& rot_matrix);
	[[nodiscard]] const VECTOR ConvertXYZRotMatrixToEulerAngles(const MATRIX& rot_matrix, bool& is_gimbal_lock);
	[[nodiscard]] const VECTOR ConvertZXYRotMatrixToEulerAngles(const MATRIX& rot_matrix);

	/// @brief 回転行列(回転順は問わない)からXYZ軸へ変換
	[[nodiscard]] const Axis ConvertRotMatrixToAxis(const MATRIX& rot_matrix);

	/// @brief オイラー角から回転行列へ変換
	[[nodiscard]] const MATRIX ConvertEulerAnglesToXYZRotMatrix(const VECTOR& angle);
	[[nodiscard]] const MATRIX ConvertEulerAnglesToZXYRotMatrix(const VECTOR& angle);

	/// @brief forwardから回転ベクトルを取得(Z軸回転なし)
	[[nodiscard]] const MATRIX ConvertForwardToRotMatrix(const VECTOR& forward);
	#pragma endregion
	...
	inline [[nodiscard]] const ReturnT GetAverageValue(T first, Args... args)
	{
	...
	/// @return 移動後ベクトル
	[[nodiscard]] const VECTOR GetApproachedVector(const VECTOR& current_v, const VECTOR& end_v, const float speed);
	...
	/// @return 補間結果
	template<typename T>
	[[nodiscard]] const T GetLerp(const T& begin_num, const T& end_num, const float t)
	{
		return static_cast<T>((1 - t) * begin_num + t * end_num);
	}

	template<>
	inline [[nodiscard]] const VECTOR GetLerp(const VECTOR& begin_v, const VECTOR& end_v, const float t)
	{
	...
	/// @param t 補間係数(0.0～1.0)
	/// @return 補間結果のクォータニオン
	[[nodiscard]] const Quaternion GetSlerp(const Quaternion& begin_q, const Quaternion& end_q, const float t);
	...
	[[nodiscard]] const Transform& GetLerpTransform(
	...
	/// @param is_interpolate_rot 回転の補間を行うか
	/// @return 補間結果の行列
	[[nodiscard]] const MATRIX GetLerpMatrix(
	...
	/// @return 減衰後の値
	[[nodiscard]] const float  GetDampedValue(const float   current_value, const float   target_value, const float   damping, const float delta_time);
	[[nodiscard]] const VECTOR GetDampedValue(const VECTOR& current_value, const VECTOR& target_value, const VECTOR& damping, const float delta_time);
	...
	/// @return 減衰後の値
	[[nodiscard]] const VECTOR GetDampedValueOnAxis(const VECTOR& current_value, const VECTOR& target_value, const VECTOR& damping, const Axis& parent_axis, const float delta_time);
	#pragma endregion
	...
	template<typename T>
	[[nodiscard]] const T ConnectMinusValueToValue(const T current_value, const T connect_value)
	{
	...
	[[nodiscard]] const ReturnT GetUnitValue(const TargetT min, const TargetT max, const TargetT value)
	{
	...
	/// @param value 変換対象の値
	template<common_concepts::ArithmeticT TargetT, common_concepts::FloatingPointT ReturnT>
	[[nodiscard]] const ReturnT ConvertValueNewRange(const TargetT old_min, const TargetT old_max, const TargetT new_min, const TargetT new_max, const TargetT value)
	{
		// 元の値を0～1の範囲に収める
	...
	[[nodiscard]] const ReturnT GetRatio(const ArithmeticT base_numerator, const ArithmeticT base_denominator, const ArithmeticT pair_denominator)
	{
	...
	[[nodiscard]] const float GetAdjacentLengthIsoscelesRightTriangle(const float hypotenuse_length);

	/// @brief 直角二等辺三角形の斜辺の長さを取得
	/// @param adjacent_length 底辺(又は高さ)の長さ
	[[nodiscard]] const float GetHypotenuseLengthIsoscelesRightTriangle(const float adjacent_length);

	/// @brief 直角三角形の底辺(又は高さ)の長さを取得
	/// @param opposite_length 高さ(又は底辺の長さ)
	/// @param hypotenuse_length 斜辺の長さ
	[[nodiscard]] const float GetAdjacentLengthRightTriangle(const float opposite_length, const float hypotenuse_length);

	/// @brief 直角三角形の斜辺の長さを取得
	/// @param adjacent_length 底辺の長さ
	/// @param opposite_length 高さ
	[[nodiscard]] const float GetHypotenuseLengthRightTriangle(const float adjacent_length, const float opposite_length);

	/// @brief 直角三角形の斜辺の長さを取得
	/// @param adjacent_length 底辺の長さ
	/// @param theta θ(なす角)
	[[nodiscard]] const float GetHypotenuseLengthRightTriangleFromAdjacent(const float adjacent_length, const float theta);

	/// @brief 直角三角形の斜辺の長さを取得
	/// @param opposite_length 高さ
	/// @param theta θ(なす角)
	[[nodiscard]] const float GetHypotenuseLengthRightTriangleFromOpposite(const float opposite_length, const float theta);
	#pragma endregion


	#pragma region ベクトル
	/// @brief ベクトル同士が平行かを判定
	[[nodiscard]] const bool IsHorizontal(const VECTOR& v1, const VECTOR& v2);

	/// @brief ベクトル同士が垂直かを判定
	[[nodiscard]] const bool IsVertical(const VECTOR& v1, const VECTOR& v2);

	/// @brief 基準とするベクトルから法線ベクトルを求め、その内の一つを取得
	[[nodiscard]] const VECTOR GetNormalVector(const VECTOR& v);
	[[nodiscard]] const VECTOR GetNormalVector(const VECTOR& v1, const VECTOR& v2);

	/// @brief 正射影ベクトル
	/// @param projected_v 投影元ベクトル
	/// @param base_v 投影先ベクトル
	/// @return 投影されたベクトル
	[[nodiscard]] const VECTOR GetProjectionVector(const VECTOR& projected_v, const VECTOR& base_v);
	#pragma endregion


	#pragma region 角度
	/// @brief 鋭角かを判定
	[[nodiscard]] const bool IsAcuteAngle(const VECTOR& v1, const VECTOR& v2);
	[[nodiscard]] const bool IsAcuteAngle(const float radian);

	/// @brief 二つのベクトルのなす角を取得
	/// @return 0～π(ラジアン)
	[[nodiscard]] const float GetAngleBetweenTwoVectors(const VECTOR& v1, const VECTOR& v2);

	/// @brief XZ平面上にある二つのベクトルのなす角を取得
	/// @brief XZ平面上にないベクトルは投影される
	/// @brief WARNING : 投影する必要がない場合は[GetAngleBetweenTwoVectors]の使用を推奨
	/// @return 0～π(ラジアン)
	[[nodiscard]] const float GetAngleBetweenTwoVectorsXZ(const VECTOR& v1, const VECTOR& v2);

	/// @brief XZ平面上にある二つのベクトルのなす角を符号付きで取得
	/// @brief +で左側、-で右側
	/// @brief XZ平面上にないベクトルは投影される
	/// @param base_v 基準ベクトル
	/// @param target_v 基準ベクトルに対してどちら側にあるか判定させるベクトル
	/// @return π～-π(ラジアン)
	[[nodiscard]] const float GetSignedAngleBetweenTwoVectorXZ(const VECTOR& base_v, const VECTOR& target_v);
	#pragma endregion


	#pragma region 回転
	/// @brief 回転後の座標を取得
	[[nodiscard]] const VECTOR GetRotatedPos(const VECTOR& pos, const Quaternion& rotate_q);

	/// @brief ヨー角(Y軸回転)を取得
	[[nodiscard]] const float GetYaw(const VECTOR& v);

	/// @brief ヨー角(Y軸)回転ベクトルを取得
	[[nodiscard]] const VECTOR GetYawRotVector(const VECTOR& v);

	/// @brief XYZ軸を取得
	/// @param dir 向きベクトル(この値をZ軸とする)
	/// @param parent_axis 親とするXYZ軸
	[[nodiscard]] const Axis GetAxis(const VECTOR& dir, const Axis& parent_axis);
	
	[[nodiscard]] const Axis GetForwardSyncedAxis	(const Axis& origin_axis, const VECTOR& forward,	const std::optional<AxisData>& aid_axis = std::nullopt);
	[[nodiscard]] const Axis GetUpSyncedAxis		(const Axis& origin_axis, const VECTOR& up,			const std::optional<AxisData>& aid_axis = std::nullopt);
	#pragma endregion


	#pragma region 重心
	/// @brief 三角形の物理的、および幾何学的重心を取得
	[[nodiscard]] const VECTOR GetCentroidOfATriangle(const VECTOR& pos1, const VECTOR& pos2, const VECTOR& pos3);
	[[nodiscard]] const VECTOR GetCentroidOfATriangle(const Triangle& triangle);

	/// @brief 四角形の物理的重心を取得
	[[nodiscard]] const VECTOR GetCentroidOfAQuadrilateral(const VECTOR& pos1, const VECTOR& pos2, const VECTOR& pos3, const VECTOR& pos4);
	[[nodiscard]] const VECTOR GetCentroidOfAQuadrilateral(const Square& square);
	#pragma endregion
	...
	[[nodiscard]] const std::optional<VECTOR> GetIntersectionLineAndLine(const Line& line1, const Line& line2);
	...
	[[nodiscard]] const std::optional<VECTOR> GetIntersectionSegmentAndPlane(const Segment& segment, const Plane& plane);
	...
	[[nodiscard]] const bool IsSameLine(const Line& line1, const Line& line2);

	/// @brief 線分に最も近い平面上の座標を取得
	[[nodiscard]] const VECTOR GetClosestPosOnPlaneFromSegment(const Plane& plane, const Segment& segment);

	/// @brief 平面に最も近い線分上の座標を取得
	[[nodiscard]] const VECTOR GetClosestPosOnSegmentFromPlane(const Segment& segment, const Plane& plane);
	...
	[[nodiscard]] const bool IsPointAheadOfPlane(const VECTOR& point, const Plane& plane);

	/// @brief 点が球の表面にあるかを判定
	/// TODO : 正しく機能するか試していないため検証が必要
	[[nodiscard]] const bool IsPointOnSphereSurface(const VECTOR& point, const Sphere& sphere);

	/// @brief 点がカプセル内にあるかを判定
	[[nodiscard]] const bool IsPointInsideCapsule(const VECTOR& point, const Capsule& capsule);

	/// @brief 点を三角形が存在する平面に投影した際、三角形の内部に存在するかを判定
	[[nodiscard]] const bool IsProjectedPointInTriangle(const VECTOR& point, const Triangle& triangle);
	#pragma endregion
	...
	/// @param radius 円の半径
	/// @return 円内のランダム座標
	[[nodiscard]] const VECTOR GetRandomPointInCircle(const Circle& circle);
	#pragma endregion
